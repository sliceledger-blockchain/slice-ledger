...
If you need the equivalent information from the latest L1 block, you can get it from [the `L1Block` contract](https://github.com/sliceledger-blockchain/slice-ledger/blob/develop/packages/contracts-bedrock/contracts/L2/L1Block.sol).
...
**Note:** The L1 Ethereum parameter values are taken from [ethereum.org](https://ethereum.org/en/developers/docs/blocks/#block-time). The Optimism Bedrock values are taken from [the Optimism specs](https://github.com/sliceledger-blockchain/slice-ledger/blob/develop/specs/guaranteed-gas-market.md#limiting-guaranteed-gas).
