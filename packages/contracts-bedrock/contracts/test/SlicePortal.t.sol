...
contract SlicePortal_Test is Portal_Initializer {
    ...
    function test_pause_onlyGuardian_reverts() external {
        ...
        vm.expectRevert("SlicePortal: only guardian can pause");
        vm.prank(alice);
        op.pause();
    ...
    function test_unpause_onlyGuardian_reverts() external {
        ...
        assertTrue(op.GUARDIAN() != alice);
        vm.expectRevert("SlicePortal: only guardian can unpause");
        vm.prank(alice);
        op.unpause();
    ...
    function test_depositTransaction_contractCreation_reverts() external {
        ...
        vm.expectRevert("SlicePortal: must send to address(0) when creating a contract");
    ...
    function test_depositTransaction_largeData_reverts() external {
        ...
        vm.expectRevert("SlicePortal: data too large");
    ...
    function test_depositTransaction_smallGasLimit_reverts() external {
        vm.expectRevert("SlicePortal: gas limit too small");
        op.depositTransaction({
            _to: address(1),
            _value: 0,
            _gasLimit: 0,
...
contract SlicePortal_FinalizeWithdrawal_Test is Portal_Initializer {
    ...
    function callPortalAndExpectRevert() external payable {
        vm.expectRevert("SlicePortal: can only trigger one withdrawal per transaction");
    ...
    function test_proveWithdrawalTransaction_paused_reverts() external {
        ...
        vm.expectRevert("SlicePortal: paused");
    ...
    function test_proveWithdrawalTransaction_onSelfCall_reverts() external {
        ...
        vm.expectRevert("SlicePortal: you cannot send messages to the portal contract");
        op.proveWithdrawalTransaction(
            _defaultTx,
    ...
    function test_proveWithdrawalTransaction_onInvalidOutputRootProof_reverts() external {
        ...
        vm.expectRevert("SlicePortal: invalid output root proof");
        op.proveWithdrawalTransaction(
            _defaultTx,
            _proposedOutputIndex,
            _outputRootProof,
    ...
    function test_proveWithdrawalTransaction_replayProve_reverts() external {
        ...
        vm.expectRevert("SlicePortal: withdrawal hash has already been proven");
        op.proveWithdrawalTransaction(
            _defaultTx,
            _proposedOutputIndex,
            _outputRootProof,
    ...
    function test_proveWithdrawalTransaction_replayProveChangedOutputRoot_succeeds() external {
        ...
        assembly {
            ...
            mstore(0x20, 52) // 52 is the slot of the `provenWithdrawals` mapping in the SlicePortal
        ...
        external
    {
        ...
        assembly {
            mstore(0x00, sload(_withdrawalHash.slot))
            mstore(0x20, 52) // 52 is the slot of the `provenWithdrawals` mapping in SlicePortal
            slot := keccak256(0x00, 0x40)
        }
    ...
    function test_finalizeWithdrawalTransaction_ifWithdrawalNotProven_reverts() external {
        ...
        vm.expectRevert("SlicePortal: withdrawal has not been proven yet");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifWithdrawalProofNotOldEnough_reverts() external {
        ...
        vm.expectRevert("SlicePortal: proven withdrawal finalization period has not elapsed");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifOutputTimestampIsNotFinalized_reverts() external {
        ...
        // Attempt to finalize the withdrawal
        vm.expectRevert("SlicePortal: output proposal finalization period has not elapsed");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_onReplay_reverts() external {
        ...
        vm.expectRevert("SlicePortal: withdrawal has already been finalized");
        op.finalizeWithdrawalTransaction(_defaultTx);
    }
    ...
    function testDiff_finalizeWithdrawalTransaction_succeeds(
        ...
        vm.assume(
            _target != address(op) && // Cannot call the slice portal or a contract
...
contract SlicePortalUpgradeable_Test is Portal_Initializer {
    ...
    function test_params_initValuesOnProxy_succeeds() external {
        SlicePortal p = SlicePortal(payable(address(proxy)));
    ...
    function test_initialize_cannotInitProxy_reverts() external {
        ...
        SlicePortal(payable(proxy)).initialize(false);
    }

    /// @dev Tests that the implementation cannot be initialized twice.
    function test_initialize_cannotInitImpl_reverts() external {
        vm.expectRevert("Initializable: contract is already initialized");
        SlicePortal(opImpl).initialize(false);
    }
